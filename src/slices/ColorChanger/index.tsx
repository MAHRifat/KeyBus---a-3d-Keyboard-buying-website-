...
const ColorChanger: FC<ColorChangerProps> = ({ slice }) => {
  ...
  return (
    ...
    >
      ...
      {/* Canvas */}
      <Canvas camera={{position: [0, .5, .5], fov: 45, zoom: 1.9}} 
      className="-mb-[10vh] grow">
        <Scene selectedTextureId={selectedTextureId} onAnimatinonComplete={handleAnimationComplete}/>
      </Canvas>
      <Bounded className="relative shrink-0" innerClassName="gap-6 lg:gap-8 flex flex-col lg:flex-row">
        <div className="max-w-md shrink-0">
          <h2 className="font-bold-slanted md-1 text-4xl lg:mb-2 lg:text-6xl">
            <PrismicText field={slice.primary.heading} />
          </h2>
          <div className="text-lg text-pretty lg:text-lg">
          ...
          <li key={texture.id}>
            ...
            className={clsx(
              ...
              <div className="mb-3 overflow-hidden rounded border-2 border-black bg-gray-100">
                <Image
                 src={texture.path} 
                 alt={texture.name} width={400} height={255}
                 className="w-full h-full object-cover"/>
              </div>
              <span className="text-sm font-semibold">{texture.name}</span>
            </button>
          </li>
        ))}
